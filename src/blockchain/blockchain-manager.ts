...
export class BlockchainManager {
  ...
  public static async generateNextBlock(blockData: object): Promise<Block> {
    ...
    const newBlock: Block = BlockchainManager.findBlock(
      nextIndex,
      previousBlock.hash,
      nextTimestamp,
      blockData,
  ...
  private static findBlock(
    ...
    return BlockchainManager.getGenesisChain()[0];
  }
  ...
  private static async getDifficulty(): Promise<number> {
    ...
    if (
      latestBlock.index % BlockchainManager.DIFFICULTY_ADJUSTMENT_INTERVAL ===
        0 &&
      latestBlock.index !== 0
    ) {
  ...
  private static async getAdjustedDifficulty(
    latestBlock: Block,
    otherChain: Block[]
  ): Promise<number> {
    const chain: Blockchain = await BlockchainManager.getChain();
    ...
    const prevAdjustmentBlock: Block =
      otherChain[
        chain.length - BlockchainManager.DIFFICULTY_ADJUSTMENT_INTERVAL
      ];

    const timeExpected: number =
      BlockchainManager.BLOCK_GENERATION_INTERVAL *
      BlockchainManager.DIFFICULTY_ADJUSTMENT_INTERVAL;
